...
class Library
{
	 ...
	 *
	 */
	runGarbageCollector(state, del = false, whitelist = [], clearResult = false)
	{
		Promise.all(this.PROMISES).then(res =>
		{
			this.clearPromiseListener(true);
			...
			this._adapter.getStates(state + '.*', (err, states) =>
			{
				...
				// compare states
				let garbage = Object.keys(states).filter(state => res.indexOf(state) == -1 && res.indexOf(state.replace(this._adapter.name + '.' + this._adapter.instance + '.', '')) == -1);
				
				// delete / empty states
				for (let index in garbage)
				{
					let state = garbage[index];
					
					// check witelist
					if (whitelist.length > 0 && RegExp(whitelist.join('|')).test(state))
						continue;
					
					// apply deletion
					this._adapter.log.debug('Garbage Collector: ' + (del ? 'Deleted ' : 'Emptied ') + state + '!');
					let key = state.substr(state.indexOf('.', state.indexOf('.')+1)+1);
					
					if (del)
					{
						this._STATES[key] = undefined;
						this.addPromiseListener(this._adapter.delObjectAsync(state));
					}
					else
	...
	_setValue(state, value, options = {})
	{
		if (state !== undefined)
		{
			if (value !== undefined && (options.force || this._STATES[state] === undefined || this._STATES[state] === null || this._STATES[state].val != value))
			{
				...
				this.addPromiseListener(this._adapter.setStateAsync(state, { val: value, ts: Date.now(), ack: true }));
			}
			else
			{
				this.setDeviceProperties(state, options.properties);
				this.addPromiseListener('resolve', state);
			}
		}
	}
	 ...
	 *
	 */
	addPromiseListener(promise, message = '')
	{
		if (['resolve', 'reject'].indexOf(promise) > -1)
	 ...
	 *
	 */
	clearPromiseListener(force)
	{
		if (force)
			this.PROMISES = [];
		
		else
			Promise.all(this.PROMISES).then(res => this.PROMISES = []);
	}
