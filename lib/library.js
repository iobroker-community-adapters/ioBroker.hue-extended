...
class Library
{
	...
	runDutyCycle(states, kill, variance)
	{
		...
		let tree = states.split('.');
		if (tree.length > 0)
		{
			that._adapter.getStatesOf(tree[tree.length-2] || '', tree[tree.length-1], (err, states) =>
			{
				states.forEach(state =>
				{
					that._adapter.getState(state._id, (err, info) =>
					{
						let ts = 0, lc = 0;
						...
						if (ts < kill && lc < kill)
						{
							that._adapter.log.warn('Duty Cycle: Deleted ' + state._id + ' (created ' + ts + ' & last change ' + lc + ' < ' + kill + ')!');
							
							that._STATES[state._id] = undefined;
