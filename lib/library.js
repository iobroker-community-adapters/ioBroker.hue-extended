...
class Library
{
	 ...
	 *
	 */
	runGarbageCollector(state, ts = null, del = true, offset = 10)
	{
		if (state === undefined)
			return false;
		...
		this._adapter.getStates(state + '.*', (err, states) =>
		{
			...
			for (let state in states)
			{
				...
				if (states[state] && (ts === null || (ts && (Math.round(states[state].lc/1000)+offset) < ts && (this._STATES[key].ts+offset) < ts)))
				{
