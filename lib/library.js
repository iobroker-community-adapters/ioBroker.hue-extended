...
class Library
{
	...
	_createNode(node, callback)
	{
		...
		this._adapter.setObjectNotExists(
			node.node,
			{
				'common': Object.assign({'name': node, 'role': 'state', 'type': 'string', 'write': false}, node.common || {}, common),
				'type': 'state',
				'native': node.native || {}
			},
			function(err, obj)
	...
	async _setValue(state, value)
	{
		...
		{
			if (this._adapter && this._adapter.log && ((this.options.updatesInLog && !this.options.updatesExceptions) || (this.options.updatesInLog && this.options.updatesExceptions && Array.isArray(this.options.updatesExceptions) && this.options.updatesExceptions.indexOf(state.substr(state.lastIndexOf('.')+1)) == -1)))
